...
This demo is built with Chroma DB and the Gemini API. Install the additional dependencies with:
...
2. Extend `./embedding.py` to replace the Gemini embedding — implement an alternative `get_*_client()` / `get_*_embedding()` pair and swap the imports in `tools.py`.
